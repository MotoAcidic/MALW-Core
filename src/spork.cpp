...
// MalwareChain: on startup load spork values from previous session if they exist in the sporkDB
...
void ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::SPORK) {
        ...
        // MalwareChain: add to spork database.
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_20_BLOCK_RECIPIENT_ENFORCEMENT) r = SPORK_20_BLOCK_ENFORCEMENT_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_20_BLOCK_RECIPIENT_ENFORCEMENT") return SPORK_20_BLOCK_RECIPIENT_ENFORCEMENT;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_20_BLOCK_RECIPIENT_ENFORCEMENT) return "SPORK_20_BLOCK_RECIPIENT_ENFORCEMENT";
