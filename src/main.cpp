...
// Copyright (c) 2017-2020 MalwareChain LLC
...
#if defined(NDEBUG)
#error "MalwareChain cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umalw) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zMALW is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zMALW spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zMALW without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zMALW spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zMALW spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMALW spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zmalw");
            }
        } else {
            LOCK(pool.cs);
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MalwareChain
...
{
    RenameThread("malwarechain-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMALWMinted()
{
...
void RecalculateZMALWSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMALW supply
...
bool RecalculateMALWSupply(int nHeightStart)
{
...
{
    // MalwareChain: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZMALWSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    UpdateZMALWSupply(block, pindex);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMALWSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight+1); //RewMod
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MALW to zMALW
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        //Additional PoS checks. yes, double check
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nVersion < 5 &&
        ...
        return state.Invalid(error("%s : rejected nVersion=4 block please update your wallet.", __func__),
            REJECT_OBSOLETE, "bad-version");
    }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zMALW spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zMALW serial %s in block\n Block: %s",
...
//    if (GetSporkValue(SPORK_17_SEGWIT_ACTIVATION) < pindexPrev->nTime) {
    if (pindexPrev != nullptr && GetSporkValue(SPORK_17_SEGWIT_ACTIVATION) < pindexPrev->nTime) {
        int commitpos = GetWitnessCommitmentIndex(block);
        if (commitpos != -1) {
            if (!IsSporkActive(SPORK_19_SEGWIT_ON_COINBASE)) {
                if (fDebug) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMALW mints and %d zMALW spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/MalwareChain Core:1.1.0/" ||
            pfrom->cleanSubVer == "/MalwareChain Core:1.3.0/" ||
            pfrom->cleanSubVer == "/MalwareChain Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // MalwareChain: We use certain sporks during IBD, so check to see if they are
